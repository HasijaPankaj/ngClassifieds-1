    ...
    <html>
      <head>
        ...
        <script type="text/javascript">
          ...
          var showIntroScene='<xsl:value-of select="/Interaction/node[@id='Show Introduction Screen']"></xsl:value-of>';
			<!-- Jyoti added flag for property box and removed flag for auto play of sound in edit mode-->
		  var dialog_flag = <xsl:value-of select = "$isPropertyBoxOpen" > </xsl:value-of>;
		  var selectedEltId = '<xsl:value-of select = "$selectedElementID" > </xsl:value-of>';
			
          <!-- var playIntroSound='<xsl:value-of select="/Interaction/node[@id='Play Introduction Sound']"></xsl:value-of>'; -->
          var maxScore = '<xsl:value-of select="/Interaction/node[@id='Max Score']"></xsl:value-of>';

          var timerType = '<xsl:value-of select="/Interaction/node[@id='Start Timer']"></xsl:value-of>';

          var n=<xsl:value-of select="/Interaction/node[@id='Number of Topics']"></xsl:value-of>;
          var totalQuestions=<xsl:value-of select="/Interaction/node[@id='Number of Questions']"></xsl:value-of>;
          ...
          var correcrAnsText = '<xsl:value-of select="$CorrectAnswer2"></xsl:value-of>';
          ...
          var yourAnsText = '<xsl:value-of select="$yourAnswer2"></xsl:value-of>';
          var TargetViewSizeFactor = <xsl:value-of select="$TargetViewSize_Factor"></xsl:value-of>;
          <xsl:choose>

            <xsl:when test="$versionId &gt;6.9">

              var gtbCount=<xsl:value-of select="/Interaction/node[@id='Number of Nodes']"></xsl:value-of>;
              var maxScore=<xsl:value-of select="/Interaction/node[@id='Max Score']"></xsl:value-of>;
              var alwaysDispInstr='<xsl:value-of select="/Interaction/node[@id='Always display the Instruction']"></xsl:value-of>';
            </xsl:when>
            <xsl:otherwise>
              var gtbCount=0;
              var maxScore=<xsl:value-of select="/Interaction/node[@id='Max Score']"></xsl:value-of>;
              var alwaysDispInstr='<xsl:value-of select="'True'"></xsl:value-of>';
            </xsl:otherwise>
          </xsl:choose>
          var versionNo=<xsl:value-of select="$versionId"></xsl:value-of>;
          var selectedOptBgColor='<xsl:value-of select="$selectedOptionColor"></xsl:value-of>';
          var coorectOptBgColor='<xsl:value-of select="$correctOptionColor"></xsl:value-of>';
          var optionColor='<xsl:value-of select="$optionColor"></xsl:value-of>';
          ...
          <xsl:choose>
            ...
            <xsl:when  test ="$AudioControlType='Upload Audio Image'">
              var OnImage = '<xsl:value-of select="$OnImage"></xsl:value-of>';
              var OffImage = '<xsl:value-of select="$OffImage"></xsl:value-of>';
            </xsl:when>
            <xsl:otherwise>
          ...
          var wavePath='<xsl:value-of select="$audioPath"></xsl:value-of>';
          var BackGroundType='<xsl:value-of select="$BackgroundType"></xsl:value-of>';
          <!-- tanaji-->
          var RtoLflag = '<xsl:value-of select="$RtoLflag"/>';
          <xsl:if test="$BackgroundType='Background Color'">
            ...
            var BackgroundColor = '<xsl:value-of select="$BackgroundColor"></xsl:value-of>';
          </xsl:if>
        ...
        <style>
          ...
          .topicStyle
          {
          font-family:<xsl:value-of select="$topicFontName"/>;
          <!--tanaji 12-6-13 appended px for IE9 support to font-size:-->
          font-size:<xsl:value-of select="$topicFontSize*$TargetViewSize_Factor"/>px;
          font-weight:<xsl:value-of select="$topicFontBold"/>;
          font-style:<xsl:value-of select="$topicFontItalic"/>;
          color:<xsl:value-of select="$topicFontColor"/>;
          }
          .base {
          width: <xsl:value-of select="$rotateWidth"/>px;
          height: <xsl:value-of select="$rotateHeight"/>px;

          left:<xsl:value-of select="$rotateLeft"/>px;
          top:<xsl:value-of select="$rotateTop"/>px;
          position:absolute;
          -moz-transform-origin: <xsl:value-of select="$rotateFactorX*$TargetViewSize_Factor"/>px  <xsl:value-of select="$rotateFactorY*$TargetViewSize_Factor"/>px;     /* move the origin to top-left */
          -webkit-transform-origin:<xsl:value-of select="$rotateFactorX*$TargetViewSize_Factor"/>px  <xsl:value-of select="$rotateFactorY*$TargetViewSize_Factor"/>px;   /* move the origin to top-left */
          -o-transform-origin: <xsl:value-of select="$rotateFactorX*$TargetViewSize_Factor"/>px  <xsl:value-of select="$rotateFactorY*$TargetViewSize_Factor"/>px;
          transform-origin:<xsl:value-of select="$rotateFactorX*$TargetViewSize_Factor"/>px  <xsl:value-of select="$rotateFactorY*$TargetViewSize_Factor"/>px;
          <!--tanaji 12-6-13 added statement for IE9 support-->
          -ms-transform-origin:<xsl:value-of select="$rotateFactorX*$TargetViewSize_Factor"/>px  <xsl:value-of select="$rotateFactorY*$TargetViewSize_Factor"/>px;
          }
      ...
      <body>
        ...
        <div id="mainPanel"  sound="{concat($audioPath,$IntroductionSoundFile)}" style="position: absolute; top: 0px; left: 0px; overflow: hidden; width:{$viewTargetWidth}px; height:{$viewTargetHeight}px">
          ...
          <xsl:call-template name="showWheel">
          ...
          <xsl:call-template name="showIntroduction">
          ...
          <xsl:call-template name="showQuestionFeedbackPanel">
          ...
          <div id="allTopics" style="display:none">
            ...
            <div id="questionPanel" style="z-index:30;display:block;position:absolute;background-size:100% 100%;background-color:{$QBackgroundColor};background-image:url('../HTML5/Wheel-QBG.png');background-repeat:no-repeat;left:{$quePanelLeft}px;top:{$quePanelTop}px;width:{$quePanelWidth}px;height:{$quePanelHeight}px;position:absolute;overflow:hidden;box-shadow:0px 1px 9px 1px #000000;-webkit-box-shadow:0px 1px 9px 1px #000000; -moz-box-shadow:0px 1px 9px 1px #000000;border-radius:15px;">
              <xsl:call-template name="showQuestionPanel">

              </xsl:call-template>


            </div>
            <div id="nonClickQuestionPanel" style="z-index:31;display:none;position:absolute;left:{$quePanelLeft}px;top:{$quePanelTop}px;width:{number($quePanelWidth)-20}px;height:{$quePanelHeight}px;position:absolute;overflow:hidden;">
            ...
            <div id="stage1Btns" style="display:block;">
              ...
              <xsl:if test="$showRestartBtn='True'">
				...
				<div id="restartButton" class=" dragThis resizeThis" style="position:absolute;top:{$rtop}px;left:{$rleft}px;width:{$rwidth}px;height:{$rheight}px;background-repeat:no-repeat;background-size:100% 100%;z-index:51;cursor:pointer;">
                  ...
                  <div id="nonClickRestart"  style=" background-size: 100% 100%; background-repeat: no-repeat;z-index:51;display:none;position:absolute;top:{$rtop}px;left:{$rleft}px;width:{$rwidth}px;height:{$rheight}px" />
				 </div>
                </xsl:if>
              </xsl:if>
			  ...
			  <div id="submitButton" class="dragThis resizeThis" style="position:absolute;top:{$topSubmit}px;left:{$leftSubmit}px;width:{$widthSubmit}px;height:{$heightSubmit}px;background-repeat:no-repeat;background-size:100% 100%;cursor:pointer;z-index:46;">
                ...
                <div id="nonClickSubmit"  style=" background-size: 100% 100%; background-repeat: no-repeat;display:block;position:absolute;top:{$topSubmit}px;left:{$leftSubmit}px;width:{$widthSubmit}px;height:{$heightSubmit}px;z-index:46;" />
			</div>
              </xsl:if>
              ...
              <xsl:if test="$showGiveUp='True'">
				...
				<div id="giveupButton" class="dragThis resizeThis" style="position:absolute;top:{$topGiveUp}px;left:{$leftGiveUp}px;width:{$widthGiveUp}px;height:{$heightGiveUp}px;background-repeat:no-repeat;background-size:100% 100%;z-index:45;cursor:pointer;">
                  ...
                  <div id="nonClickGiveUp"  style=" background-size: 100% 100%; background-repeat: no-repeat;z-index:46;display:none;position:absolute;top:{$topGiveUp}px;left:{$leftGiveUp}px;width:{$widthGiveUp}px;height:{$heightGiveUp}px" />
				 </div>
                </xsl:if>
              </xsl:if>

              <xsl:call-template name="displayScorePanel">
          ...
          <xsl:call-template name="CreateFinalFeedback">
          ...
          <!--<div id="TitleText" class="txtControl" style="overflow:hidden;white-space:nowrap;position:absolute;z-index:25;top:{$topTitle}px;left:{$leftTitle}px;width:{$widthTitle}px;height:{$heightTitle}px;">
              <xsl:value-of select="$TitleText" disable-output-escaping="yes" />
            </div>
            -->
          <!-- Closing Title -->
          <!--
          ...
          <!-- Instruction text -->
          <!--
            <div id="InstructionText" class="txtControl"  style="display:none;overflow:hidden;word-wrap: break-word;position:absolute;z-index:25;top:{$topInstruction}px;left:{$leftInstruction}px;width:{$widthInstruction}px;height:{$heightInstruction}px;">
...
<!-- Global Functions starts -->
  ...
  <xsl:template name="showWheel">
    ...
    <div id="wheelDiv" style="z-index:30;display:none;left:{$wheelLeft}px;top:{$wheelTop}px;position:absolute;">
      ...
      <div style="position:absolute;left:{$wheelImgLeft}px;top:{$wheelImgTop}px;background-image:url({$wheelImg});background-repeat:no-repeat;background-size:100% 100%;width:{$wheelImgWidth}px;height:{$wheelImgHeight}px;z-index:35;">
      </div>


      <div id="rotateDiv" style="background-image:url('../HTML5/Wheel-arrow.png');background-repeat:no-repeat;background-size:100% 100%;z-index:40;" class="base rotate_translate">
      </div>
      ...
      <div id="nonClickhitBtn"  style="display:none;position:absolute;left:{$hitBtnLeft}px;top:{$hitBtnTop}px;width:{$hitBtnWidth}px;height:{$hitBtnHeight}px;cursor:default;z-index:51;">
  ...
  <xsl:template name="showTopicNames">
    ...
    <xsl:if test="$totalTopics='6'">
      ...
      <!--<div class="txtControl topicStyle" id="topic5" style="width:83px;height:37px;left:40px;top:125px;position:absolute;z-index:35;overflow:hidden;word-wrap: break-word;">
        <xsl:value-of select="/Interaction/node[@id='Topic 6 Title']" disable-output-escaping="yes"/>
    ...
    <xsl:if test="$totalTopics='4'">
      ...
      <!--<div class="txtControl topicStyle" id="topic3" style="width:90px;height:37px;left:30px;top:195px;position:absolute;z-index:35;overflow:hidden;word-wrap: break-word;">
        <xsl:value-of select="/Interaction/node[@id='Topic 5 Title']" disable-output-escaping="yes"/>
  ...
  <xsl:template name="getWheelInfo">
    ...
    <xsl:if test="$CurrentIndex &lt;= $LastIndex">
      ...
      partsColor[<xsl:value-of select="number($CurrentIndex)-1" />]='<xsl:value-of select="$BgColor"/>';
      topic[<xsl:value-of select="number($CurrentIndex)-1" />]='<xsl:value-of select="$topicName"/>';
      orgTopic[<xsl:value-of select="number($CurrentIndex)-1" />]='<xsl:value-of select="/Interaction/node[@id=concat(concat('Topic ',number($CurrentIndex)),' Title')]" />';
      <xsl:call-template name ="getWheelInfo">
  ...
  <xsl:template name ="showQuestionFeedbackPanel">
    ...
    <div  id="optionNonClickDiv"  style="display:none;top:{$optionTop}px;left:{$optionLeft}px;width:402px;height:93px;position:absolute;z-index:61;">
    </div>
  ...
  <xsl:template name="displayScorePanel">
    ...
    <div id="scorePanel" class = "dragThis" style="z-index:30;cursor:default;display:block;position:absolute;background-image:url('../HTML5/Wheel-score.png');background-repeat:no-repeat;background-size:100% 100%;top:{$ScorePnltop}px;left:{$ScorePnlleft}px;width:{110*$TargetViewSize_Factor}px;height:{30*$TargetViewSize_Factor}px">
      ...
      <!--<div id="scoreHeading" class="txtControl"  style="cursor:text;display:block;position:absolute;background-repeat:no-repeat;background-size:100% 100%;background-image:url('score_head.png');background-color:{$scoreHeadingColor};top:3px;left:3px;width:98px;height:24px;overflow: hidden;">
        <div class="txtControl" style="top:3px;width:98px;height:24px;position:absolute;overflow: hidden;">
          <xsl:value-of select ="/Interaction/node[@id='Score Heading']" disable-output-escaping="yes" />
        </div>
      </div>-->
      ...
      <div class="txtControl" id="FinalFedbkTxtContainer" style="display:block;position:absolute; width:{$endFdbkContWidth}px; height:{$endFdbkContHeight}px; top:{$endFdbkContTop}px; left:{$endFdbkContLeft}px;overflow:hidden;z-index:10;">
        ...
        <div id="FinalFedbkScrollContainer" class="Scroller-Container">
          ...
          <div id="FinalFeedback" style="display:block;position:absolute; width:{$endFdbkTxtWidth}px;top:0px; left:{$endFdbkTxtLeft}px">
            ...
            <!--<xsl:variable name="correctSoundFile"  select="/Interaction/node[@id='Correct Feedback Sound']"></xsl:variable>
            <div id="correctFinalFeedback" class="txtControl"  sound="{concat($audioPath,$correctSoundFile)}" style="display:none;">
              <xsl:value-of select="/Interaction/node[@id='Correct Feedback']" disable-output-escaping="yes" />
            </div>

            <xsl:variable name="incorrectFedbkSoundFile" select="/Interaction/node[@id='Incorrect Feedback Sound']"></xsl:variable>
            <div id="incorrectFinalFeedback" class="txtControl" sound="{concat($audioPath,$incorrectFedbkSoundFile)}" style="display:none;">
              <xsl:value-of select="/Interaction/node[@id='Incorrect Feedback']" disable-output-escaping="yes" />
            </div>-->
            ...
            <!--<xsl:call-template name="CreateTopicWiseTitles">
              <xsl:with-param name="topicNo" select="1"></xsl:with-param>
              <xsl:with-param name="totalTopics" select="/Interaction/node[@id='Number of Topics']"></xsl:with-param>
            ...
            <div dir="{$direction}" id="checkAnswerDiv" class="txtControl" style="text-align:{$setAlign};font-family:{$fontName};font-size:{$fontSize*$TargetViewSize_Factor}px;font-weight:{$fontBold};font-style:{$fontItalic}; color:{$fontColor}; position:relative;">
            </div>
        ...
        <!--<xsl:variable name="endOfGameSoundFile" select="/Interaction/node[@id='End of Game Feedback Sound']"></xsl:variable>
      <div id="endOfGameFeedback" class="txtControl"  style="display:none;position:absolute;left:10px;top:10px;width:300px;height:140px;overflow:hidden;"  sound="{concat($audioPath,$endOfGameSoundFile)}">
        <xsl:value-of select="/Interaction/node[@id='End of Game Feedback']" disable-output-escaping="yes" />
      </div>-->
      </div>
  ...
  <!--<xsl:template name="DisplayImage">
    ...
    <xsl:variable name="TargetWidth" select="$width" />
    <xsl:variable name="TargetHeight" select="$height"/>
    <xsl:variable name="widthHeight">
      <xsl:call-template name="ImageScale">
        <xsl:with-param name="Target_Width" select="$TargetWidth"></xsl:with-param>
        <xsl:with-param name="Target_Height" select="$TargetHeight"></xsl:with-param>
    ...
    <img id="{$controlId}" class="MAR" imageSetting="{$ImageSetting}${$TargetWidth*$TargetViewSize_Factor}${$TargetHeight*$TargetViewSize_Factor}" src="{concat($picPath,$path)}" alt="" style="position:{$position};top:{$top}px;left:{$left}px;width:{$ImgWidth}px;height:{$ImgHeight}px; background-repeat: no-repeat;margin:0 0 0 0 0;border-style:none" />
  </xsl:template>-->
